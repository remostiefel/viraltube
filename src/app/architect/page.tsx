...
import { processChat, generateScriptAction, saveTemplateAction, getTemplatesAction, deleteTemplateAction, generateBlueprintAction, saveScriptToVaultAction, getProjectByIdAction, updateProjectAction, synthesizeStrategyAction, refineScriptAction, extendScriptAction } from "@/app/actions";
...
import { generateVideoPromptsAction, generateScriptImagePromptsAction, generateAudioPromptsAction } from "@/app/actions";
...
export default function Architect() {
    ...
    const handleScriptGenerate = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const data = await generateScriptAction(scriptTopic, targetLanguage, contextToUse);
    ...
    const handleSaveTemplate = async () => {
        // PREFER CONTENT (The Editor)
        // If content (editor) is empty, fall back to AI result text if present, or empty.
        const textToSave = content || (scriptResult ? scriptResult.sections.map(s => s.content).join('\n') : "");

        if (!textToSave.trim()) {
            return; // Silently fail if empty (Safety)
        }

        if (savingTemplate === "saving") return;
        setSavingTemplate("saving");

        try {
            // Determine Name
            let name = scriptResult?.title || scriptTopic || "Untitled Draft";

            // If we have a current ID, maybe we should update? 
            // For now, we ALWAYS save as new snapshot to preserve history, unless we implement explicit "Overwrite".
            if (!name.includes("(Draft)")) {
                const time = new Date().toLocaleTimeString('de-DE', { hour: '2-digit', minute: '2-digit' });
                name = `${name} (Draft ${time})`;
            }

            await saveTemplateAction("script", name, textToSave, projectId || undefined); // Pass projectId if available!
            await loadTemplates();
    ...
    const handleLoadTemplate = (template: Template) => {
        ...
        if (isBlueprint && typeof template.content === 'string') {
            ...
            setScriptResult({ title: template.name, sections: [] }); // Set title context
    ...
    const handleSaveToVault = async () => {
        ...
        if (!configStr) {
            alert("Please configure your Cloud Vault in Settings first.");
            return;
        }
        ...
        <div className="flex flex-col h-[calc(100vh-8rem)] max-w-6xl mx-auto">
            ...
            {activeTab === "strategy" && (
                <div className="flex-1 grid grid-cols-2 gap-6 overflow-hidden">
                    ...
                    <div className="bg-card/50 border border-purple-500/20 rounded-xl p-6 overflow-y-auto">
                    ...
                    <div className="flex flex-col bg-background border border-purple-500/30 rounded-xl shadow-lg overflow-hidden">
                        <div className="flex justify-between items-center p-4 border-b border-border/40 bg-muted/20">
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                >
                                    ...
                                    {savingBlueprintStatus === "saved" ? "Saved!" : "Save as Blueprint"}
                                </button>
                                ...
                                <button
                                    ...
                                    disabled={savingStrategyStatus === "saving"}
                                    className={cn("px-3 py-1.5 rounded-md text-xs font-bold transition-all flex items-center gap-2",
                                        savingStrategyStatus === "saved" ? "bg-green-500 text-white" : "bg-purple-600 hover:bg-purple-700 text-white"
                                    )}
                                >
                                    {savingStrategyStatus === "saving" ? <Loader2 className="w-3 h-3 animate-spin" /> :
                                        savingStrategyStatus === "saved" ? <CheckCircle2 className="w-3 h-3" /> : <Save className="w-3 h-3" />}
                                    {savingStrategyStatus === "saved" ? "Saved!" : "Save Strategy"}
                                </button>
                            </div>
                        </div>
                        <textarea
                        ...
                        <div className="bg-card border border-border/40 rounded-xl p-6 shadow-lg flex items-start gap-4">
                            <form onSubmit={handleScriptGenerate} className="flex-1 flex gap-4 items-center">
                                ...
                                <button
                                    ...
                                    className="bg-primary text-background font-bold px-6 py-3 rounded-lg hover:bg-primary/90 disabled:opacity-50 transition-colors flex items-center gap-2 min-w-[140px] justify-center h-[52px]"
                                >
                                    {scriptLoading ? <Loader2 className="w-5 h-5 animate-spin" /> : <PencilRuler className="w-5 h-5" />}
                                    Architect
                                </button>
                            </form>
                        </div>
                                                ...
                                                <div className="flex flex-col xl:flex-row xl:items-center justify-between mb-4 gap-4">
                                                    ...
                                                    <div className="flex flex-wrap gap-2 justify-end">
                                                        <button
                                                            ...
                                                            className={cn("flex items-center gap-1 px-3 py-1 text-xs font-bold uppercase tracking-widest rounded border transition-colors",
                                                        ...
                                                        <button
                                                            ...
                                                            className="flex items-center gap-1 px-3 py-1 text-xs font-bold uppercase tracking-widest rounded border border-blue-500/30 text-blue-500 hover:bg-blue-500/10 transition-colors"
                                                        >
                                                        ...
                                                        <button
                                                            ...
                                                            className="flex items-center gap-1 px-3 py-1 text-xs font-bold uppercase tracking-widest rounded border border-green-500/30 text-green-500 hover:bg-green-500/10 transition-colors"
                                                        >
                                                        ...
                                                        {strategyContent && (
                                                            <button
                                                                ...
                                                                className="flex items-center gap-1 px-3 py-1 text-xs font-bold uppercase tracking-widest rounded border border-amber-500/30 text-amber-500 hover:bg-amber-500/10 transition-colors"
                                                            >
                                                        ...
                                                        <button
                                                            ...
                                                            className="flex items-center gap-1 px-3 py-1 text-xs font-bold uppercase tracking-widest rounded border border-indigo-500/30 text-indigo-500 hover:bg-indigo-500/10 transition-colors"
                                                        >
